...
CLI usage:
    ...
    python3 plan-utils.py find-for-session <project_root> <ppid>
    python3 plan-utils.py find-unclaimed <project_root>
"""
...
def update_step_status(plan_path, step_id, new_status):
    ...
    result field is empty. These are soft warnings; higher-level guards
    remain the hard gate for completion.
    """
    plan = read_plan(plan_path)
    ...
    if new_status == "done":
        incomplete = [
            progress_item["task"] for progress_item in step.get("progress", [])
            if progress_item.get("status") != "done"
        ]
        if incomplete:
            print(
