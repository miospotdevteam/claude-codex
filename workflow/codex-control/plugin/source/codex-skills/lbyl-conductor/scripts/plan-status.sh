...
session_label_for_dir() {
  local dir="$1"
  local lock_file="$dir/.session-lock"
  ...
  if [ -n "$CURRENT_SESSION_PID" ] && [ "$owner" = "$CURRENT_SESSION_PID" ]; then
    echo "THIS SESSION"
    return 0
  fi
...
show_plan_json() {
  ...
  local label="${2:-}"
  local owner_label
  owner_label="$(session_label_for_dir "$dir")"

  echo "========================================"
  if [ -n "$label" ]; then
    echo "  Plan: $name  [$label | $owner_label]"
  else
    echo "  Plan: $name  [$owner_label]"
  fi
  echo "----------------------------------------"
