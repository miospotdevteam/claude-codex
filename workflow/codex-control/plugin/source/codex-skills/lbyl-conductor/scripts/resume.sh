...
if [[ "${1:-}" == "--session" ]]; then
  if [[ -z "${2:-}" ]]; then
    echo "Usage: bash resume.sh [--session <ppid>]"
    exit 1
  fi
  SESSION_PID="$2"
fi
...
PLAN_UTILS="$SCRIPT_DIR/plan_utils.py"
...
if [ -n "$SESSION_PID" ]; then
  latest_json=$(python3 "$PLAN_UTILS" find-for-session "$PROJECT_ROOT" "$SESSION_PID" 2>/dev/null) || true
fi

if [ -z "$latest_json" ]; then
  unclaimed=$(python3 "$PLAN_UTILS" find-unclaimed "$PROJECT_ROOT" 2>/dev/null | head -1) || true
  if [ -n "$unclaimed" ]; then
    latest_json=$(printf '%s' "$unclaimed" | awk -F '\t' '{print $2}')
    if [ -n "$latest_json" ] && [ -n "$SESSION_PID" ]; then
      echo "$SESSION_PID" > "$(dirname "$latest_json")/.session-lock"
    fi
  fi
fi

if [ -z "$latest_json" ] && [ -n "$SESSION_PID" ]; then
  claimed_active=$(python3 "$PLAN_UTILS" find-active "$PROJECT_ROOT" 2>/dev/null) || true
  if [ -n "$claimed_active" ]; then
    echo "No resumable plan is claimed for session $SESSION_PID."
    echo "Active plans may belong to other sessions."
    echo "Inspect them with: bash .temp/plan-mode/scripts/plan-status.sh"
    exit 0
  fi
fi
