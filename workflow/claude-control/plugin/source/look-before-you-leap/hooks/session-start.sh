...
json_field() {
  ...
  JSON_FIELD_PAYLOAD="$payload" JSON_FIELD_KEY="$key" python3 <<'PYEOF'
import json
import os

value = json.loads(os.environ["JSON_FIELD_PAYLOAD"]).get(os.environ["JSON_FIELD_KEY"], "")
...
PYEOF
    ...
    if [ -n "$plan_status_info" ]; then
      done_count=$(json_field "$plan_status_info" "done" 2>/dev/null) || true
      active_count=$(json_field "$plan_status_info" "active" 2>/dev/null) || true
      pending_count=$(json_field "$plan_status_info" "pending" 2>/dev/null) || true
      blocked_count=$(json_field "$plan_status_info" "blocked" 2>/dev/null) || true
      next_step=$(json_field "$plan_status_info" "next_step" 2>/dev/null) || true
      has_work=$(json_field "$plan_status_info" "has_work" 2>/dev/null) || true
    fi
...
PYEOF
      ...
      if [ -n "$other_status_info" ]; then
        done_count=$(json_field "$other_status_info" "done" 2>/dev/null) || true
        active_count=$(json_field "$other_status_info" "active" 2>/dev/null) || true
        pending_count=$(json_field "$other_status_info" "pending" 2>/dev/null) || true
        blocked_count=$(json_field "$other_status_info" "blocked" 2>/dev/null) || true
        next_step=$(json_field "$other_status_info" "next_step" 2>/dev/null) || true
      fi
...
routing_context = (
    "**Codex Orchestration (call Codex CLI directly via shell scripts):**\n\n"
    "You delegate work to OpenAI Codex CLI by calling shell scripts directly\n"
    "via Bash. There are NO agent `.md` wrappers — you build the prompt and\n"
    ...
    "**How to call Codex (worker):**\n"
    "1. Build a self-contained prompt (Codex has no parent context)\n"
    "2. Write the prompt to a temp file for long prompts\n"
    "3. Call via Bash:\n"
    "```bash\n"
    f"{codex_worker_script} --prompt-file /tmp/codex-prompt.txt --output /tmp/codex-result.txt\n"
    "```\n"
    "4. Read the output file to see what Codex produced\n"
    "5. For parallel tasks, dispatch multiple Bash calls simultaneously\n\n"
    "**How to call Codex (verifier):**\n"
    "1. Write requirements to a temp file\n"
    "2. Call via Bash:\n"
    "```bash\n"
    f"{codex_verifier_script} --requirements-file /tmp/requirements.txt --output /tmp/codex-verify.txt\n"
    "```\n"
    "3. Read the output for VERDICT: PASS/FAIL\n\n"
    "**Auth:** Prefer subscription auth. Run `codex login status` to check.\n"
    "Use `codex login` if needed. Do not assume `OPENAI_API_KEY` is present.\n\n"
